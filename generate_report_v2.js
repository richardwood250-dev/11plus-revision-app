    ...
    if (file === 'verbal.js') {
        ...
        function processArray(arr, defaultTopic) {
            ...
            for (const q of arr) {
                ...
                if (file === 'nonverbal.js') {
                    // For NVR, use image name as unique reference
                    if (q.image) {
                        try {
                            const urlObj = new URL(q.image);
                            let filename = path.basename(urlObj.pathname);

                            // Let's strip the extension
                            let base = filename.replace(/\.[^/.]+$/, "");

                            // strip trailing _number, _qNumber, _var_number
                            const match = base.match(/(.*?)(?:_var_|_q|_)\d+$/i);
                            if (match) {
                                uniqueId = match[1];
                            } else {
                                uniqueId = base;
                            ...
                            // in case it's not a valid URL or doesn't have an image
                            uniqueId = q.image;
                        }
                    } else {
                        uniqueId = q.id;
...
// Write to active workspace instead of artifact path
const reportPath = 'C:\\Users\\richa\\OneDrive\\Documents\\Github repo\\11plus-revision-app\\question_counts_report.md';
