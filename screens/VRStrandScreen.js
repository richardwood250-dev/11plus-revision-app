...
import { getVRStrandBelts } from '../utils/vrSkills';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { getDojoRecords } from '../utils/storage';

export const VRStrandScreen = () => {
    ...
    const belts = getVRStrandBelts(strandId);
    ...
    const renderBelt = (belt) => {
        return (
            <TouchableOpacity 
                ...
                onPress={() => navigation.navigate('VRSkillsQuiz', { 
                    strandId: strandId,
                    beltId: belt.id, 
                    beltName: belt.name, 
            ...
            >
                <View style={styles.beltContent}>
                    ...
                    {records[`vr_${strandId}_${belt.id}`] && (
                        <Text style={[styles.beltRecord, { color: belt.text }]}>
                            🏆 Best: {records[`vr_${strandId}_${belt.id}`].bestScore}/{records[`vr_${strandId}_${belt.id}`].maxScore}   ⏱️ {formatTime(records[`vr_${strandId}_${belt.id}`].bestTime)}
                        </Text>
                    )}
                </View>
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                <View style={styles.introContainer}>
                    <Text style={styles.introTitle}>Choose Your Challenge</Text>
                    <Text style={styles.introText}>
                        Master each rank to improve your verbal reasoning agility. Expert Black Belt level is designed to challenge even the most advanced vocabularies!
                    </Text>
                </View>
